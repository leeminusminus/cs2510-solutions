...
// to represent a stop on a bus route
class BusStation implements IStation {
	String name;
	String line;
	boolean express;
	
	BusStation(String name, String line, boolean express){
		this.name = name;
		this.line = line;
		this.express = express;
	}
...
class ExamplesIStation{
  ...
  Cary Depot on the 310 line is not an express stop
  GoRaleigh Station on the DRX Line is an express stop
  Regional Transit Center is on the 800 Line is not an express stop
   */
  ...
  IStation cary =
  		new BusStation("Cary Depot", "310", false);
  IStation goraleigh =
  		new BusStation("GoRaleigh Station", "DRX", true);
  IStation regional =
  		new BusStation("Regional Transit Center", "800", false);
}