...
class Square extends Rect {
  ...
  @Override
  public IShape grow(int inc) {
    return new Square(this.loc, this.height + inc, this.color);
  }
}
...
class Rect extends AShape {
  ...
  public boolean contains(CartPt pt) {
    return
        pt.x >= this.loc.x && pt.x <= this.loc.x + width &&
        pt.y >= this.loc.y && pt.y <= this.loc.y + height;
  }
}
...
// returns the distance between a given point and this point
  double distTo(CartPt that) {
    return Math.sqrt((this.x - that.x) * (this.x - that.x) + (this.y - that.y) * (this.y - that.y));
  }
}

// represents examples and tests of IShape
class ExamplesShape {
  ExamplesShape() {
  }
